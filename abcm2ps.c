...
INFO info;
struct SYMBOL *sym;  /* symbols of the current voice */
...
int epsf;                  /* 1: EPSF, 3: embedded ABC */
...
static int str_level;  /* current area level */
static struct str_a {
...
FILE *open_file(char *fn, char *ext, char *rfn)
{
  ...
  if (in_fname && in_fname != fn && (p = strrchr(in_fname, DIRSEP)) != NULL) {
    ...
    strcpy(&rfn[l], fn);
    if ((fp = open_ext(rfn, ext)) != NULL)  { return fp; }
  }
...
/* return the real/full file name in tex_buf[] */  // TODO: fix indentation from here!
