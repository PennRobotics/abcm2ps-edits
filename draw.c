...
void set_color(int new_color)
{
	...
	cur_color = new_color;
	a2b("%.2f %.2f %.2f setrgbcolor ", (float)(cur_color>>16)/255, (float)((cur_color>>8)&0xff)/255, (float)(cur_color&0xff)/255);
}
...
{
	return ((int)(y + 2.999) + 12) / 6 * 6 - 12 - y;
}
...
{
	struct SYMBOL *s = (struct SYMBOL *)getarena(sizeof *s);
	memcpy(s, s_orig, sizeof *s);
	s->flags |= ABC_F_INVIS;
	s->text = NULL;
	for (int m = 0; m <= s->nhd; m++)
...
static int calculate_beam(struct BEAM *bm, struct SYMBOL *s1)
{
	...
	for (s2 = s1; ; s2 = s2->next) {
		if (s2->abc_type == ABC_T_NOTE) {
			...
			if (s2->stem != s1->stem)
				two_dir = 1;
			if (!visible && !(s2->flags & ABC_F_INVIS) && (!(s2->flags & ABC_F_STEMLESS) || (s2->sflags & S_TREM2)))
				visible = 1;
...
if (s1->xs == s2->xs)
 bug("beam with null length", 1);
	...
	if (two_dir) {  /* 2 directions */
		...
		while (s->ts_prev->abc_type == ABC_T_NOTE && s->ts_prev->time == s->time && s->ts_prev->x > s1->xs)
			s = s->ts_prev;

		for (; s && s->time <= s2->time; s = s->ts_next) {
			if (s->abc_type != ABC_T_NOTE || (s->flags & ABC_F_INVIS) || (s->staff != staff && s->voice != voice))
				continue;
	...
	if (a == 0)  /* shift flat beams onto staff lines */
		b += b_pos(s1->flags & ABC_F_GRACE, s1->stem, nflags, b - staff_tb[staff].y);
...
/* beam number (1..n) */
static void draw_beam(float x1, float x2, float dy, float h, struct BEAM *bm, int n)
{
	struct SYMBOL *s;
	float y1, dy2;
...
static void draw_beams(struct BEAM *bm)
{
	...
	for (s = s1; ; s = s->next) {
		if (s->abc_type == ABC_T_NOTE && s->stem != beam_dir)
			s->ys = bm->a * s->xs + bm->b - staff_tb[s->staff].y + bshift * (s->nflags - 1) * s->stem - bh;
		if (s == s2)
			break;
	}
	...
	for (i = 2; i <= bm->nflags; i++) {
		...
		for (s = s1; ; s = s->next) {
			...
			for (;;) {
				...
				if ((s->next->sflags & S_BEAM_BR1) || ((s->next->sflags & S_BEAM_BR2) && i > 2))
					break;
				s = s->next;
			}
			k2 = s;
			...
			if (k1 == k2) {
				if (k1 == s1) {
					...
					x1 -= bstub;
				} else if ((k1->sflags & S_BEAM_BR1) || ((k1->sflags & S_BEAM_BR2) && i > 2)) {
					x1 += bstub;
				} else {
					struct SYMBOL *k;
...
static void draw_sysbra(float x, int staff, int flag)
{
	...
	a2b("%.1f %.1f %.1f %s\n", yt - yb, x, yt, (flag & (CLOSE_BRACE | CLOSE_BRACE2)) ? "brace" : "bracket");
}
...
// draw helper lines
static void draw_hl(float x, float staffb, int up, int y, char *stafflines, char *hltype)
{
	int i, l;
...
static void draw_keysig(struct VOICE_S *p_voice, float x, struct SYMBOL *s)
{
	...
	if (s->u.key.nacc == 0 && !s->u.key.empty) {
		...
		for (i = 0; i < s->u.key.nacc; i++) {
			acc = s->u.key.accs[i];
			// (clef shift)
			shift = s->u.key.clef_delta * 3 + 3 * (s->u.key.pits[i] - 18);
			...
			if (shift < 0)
				/* (lower ledger line) */
				draw_hl(x, staffb, 0, shift, staff_tb[s->staff].stafflines, "hl");
			else if (shift > 24)
				/* upper ledger line */
				draw_hl(x, staffb, 1, shift, staff_tb[s->staff].stafflines, "hl");
			last_shift = shift;
			putxy(x, staffb + shift);
...
static void draw_bar(struct SYMBOL *s, float bot, float h)
{
	...
	if (staff != 0 && s->ts_prev && s->ts_prev->type != BAR)
...
static void draw_rest(struct SYMBOL *s)
{
	...
	if (s->dur == voice_tb[s->voice].meter.wmeasure || ((s->sflags & S_REPEAT) && s->doty >= 0)) {
		struct SYMBOL *s2;
...
static void draw_gracenotes(struct SYMBOL *s)
{
	...
	a2b("%.2f %.2f %.2f %.2f %.2f %.2f ", x1-x0, y1-y0, x2-x0, y2-y0, x3-x0, y3-y0);
...
/* (the staves are defined) */
static void draw_basic_note(float x, struct SYMBOL *s, int m, signed char *y_tb)
{
	 ...
	 * and note on a line */
	if (y % 6 == 0 && shhd != (s->stem > 0 ? s->u.note.notes[0].shhd : s->u.note.notes[s->nhd].shhd)) {
		int yy;
	...
	if (note->invisible) {
		...
		switch (head) {
		case H_OVAL:
			if (note->len < BREVE)  { p = "HD"; break; }
			if (s->head != H_SQUARE)  { p = "HDD"; break; }
			/* fall thru */
		case H_SQUARE:
...
static void draw_note(float x, struct SYMBOL *s, int fl)
{
	...
	if (!(s->flags & ABC_F_INVIS)) {
		...
		shhd = (s->stem > 0 ? s->u.note.notes[0].shhd : s->u.note.notes[s->nhd].shhd) * cur_scale;
		if (s->pits[0] < 22)
			/* lower ledger lines */
			draw_hl(x + shhd, staffb, 0, 3 * (s->pits[0] - 18), staff_tb[s->staff].stafflines, hltype);
		if (s->pits[s->nhd] > 22)
			/* upper ledger lines */
			draw_hl(x + shhd, staffb, 1, 3 * (s->pits[s->nhd] - 18), staff_tb[s->staff].stafflines, hltype);
	}
	...
	if (!(s->flags & ABC_F_INVIS) && fl && (s->sflags & S_TREM1)) {
		...
		x1 = x + (s->stem > 0 ? s->u.note.notes[0].shhd : s->u.note.notes[s->nhd].shhd) * cur_scale;
...
static struct SYMBOL *next_scut(struct SYMBOL *s)
{
	...
	for (s = s->next; s; s = s->next) {
		if (s->type == BAR && ((s->sflags & S_RRBAR) || s->u.bar.type == B_THIN_THICK || s->u.bar.type == B_THICK_THIN || (s->u.bar.repeat_bar && s->text && s->text[0] != '1')))
			return s;
		prev = s;
	}
	return prev;
...
/* -- output a slur / tie -- */
/* if < 0, the staves are defined */
static void slur_out(float x1, float y1, float x2, float y2, int s, float height, int dotted, int staff)
{
	...
	if (!dotted)
		a2b("%.2f %.2f %.2f %.2f %.2f %.2f 0 %.2f ", (xx2 - dx - x2)/cur_scale, (yy2 + dy - y2 - dz)/scale_y, (xx1 + dx - x2)/cur_scale, (yy1 + dy - y2 - dz)/scale_y, (x1 - x2)/cur_scale, (y1 - y2 - dz)/scale_y, dz);
	a2b("%.2f %.2f %.2f %.2f %.2f %.2f ", (xx1 - x1)/cur_scale, (yy1 - y1)/scale_y, (xx2 - x1)/cur_scale, (yy2 - y1)/scale_y, (x2 - x1)/cur_scale, (y2 - y1)/scale_y);
	putxy(x1, y1);
	if (staff >= 0)
		a2b("y%d ", staff);
	a2b(dotted ? "dSL\n" : "SL\n");
...
static int draw_slur(struct SYMBOL *k1_orig, struct SYMBOL *k2, int m1, int m2, int slur_type)
{
	struct SYMBOL *k1, *k;
	...
	if (m1 >= 0) {
		...
		if (k1->abc_type == ABC_T_NOTE) {
			if (s > 0) {
				if (k1->stem > 0) {
					x1 += 5;
					if ((k1->sflags & S_BEAM_END) && k1->nflags >= -1 /* stemmed */ && (!(k1->sflags & S_IN_TUPLET))) {
						if (k1->nflags > 0) {
							x1 += 2;
							y1 = k1->ys - 3;
						} else {
				...
				if (k1->stem < 0) {
					x1 -= 1;
					if ((k1->sflags & S_BEAM_END) && k1->nflags >= -1 && (!(k1->sflags & S_IN_TUPLET) || k1->ys < y1 + 3)) {
						if (k1->nflags > 0) {
							x1 += 2;
							y1 = k1->ys + 3;
						} else {
	...
	if (m2 >= 0) {
		...
		if (k2->abc_type == ABC_T_NOTE) {
			if (s > 0) {
				if (k2->stem > 0) {
					x2 += 1;
					if ((k2->sflags & S_BEAM_ST) && k2->nflags >= -1 && (!(k2->sflags & S_IN_TUPLET)))
						y2 = k2->ys - 6;
					else
						y2 = k2->ys + 3;
				} else {
...
static void draw_slurs(struct SYMBOL *first, struct SYMBOL *last)
{
	...
	for (;;) {
		...
		if ((s->type != NOTEREST && s->type != SPACE) || (s->u.note.slur_st == 0 && !(s->sflags & S_SL1))) {
			s = s->next;
			continue;
		}
		...
		for (;;) {
			...
			if (s1->type == BAR && ((s1->sflags & S_RRBAR) || s1->u.bar.type == B_THIN_THICK || s1->u.bar.type == B_THICK_THIN || (s1->u.bar.repeat_bar && s1->text && s1->text[0] != '1'))) {
				k = s1;
				break;
			}
		...
		m2 = -1;
		cont = 0;
		if ((k->type == NOTEREST || k->type == SPACE) && (k->u.note.slur_end || (k->sflags & S_SL2))) {
			...
			if (k->type != BAR || (!(k->sflags & S_RRBAR) && k->u.bar.type != B_THIN_THICK && k->u.bar.type != B_THICK_THIN && (!k->u.bar.repeat_bar || !k->text || k->text[0] == '1')))
				cont = 1;
		}
...
/* (tuplet in extra) */
static struct SYMBOL *draw_tuplet(struct SYMBOL *t, struct SYMBOL *s)  /* main note */
{
	...
	for (s2 = s; s2; s2 = s2->next) {
		...
		if (s2->u.note.slur_st  /* if slur start/end */ || s2->u.note.slur_end || (s2->sflags & (S_SL1 | S_SL2)))
			some_slur = 1;
		if (s2->staff < upstaff)
			upstaff = s2->staff;
		if (!s1)
	...
	if (s1 == s2) {  /* tuplet with 1 note (!) */
		...
		// TODO: enums
		if ((t->aux & 0xf000) == 0x2000  /* if 'when' == always */ || s1->abc_type != ABC_T_NOTE || s2->abc_type != ABC_T_NOTE) {
			nb_only = 0;
		} else {
			nb_only = 1;
			...
			if (nb_only && !(s2->sflags & S_BEAM_END)) {
				for (sy = s2->next; sy; sy = sy->next) {
					if (sy->type == NOTEREST) {
						if (!(sy->sflags & (S_BEAM_BR1 | S_BEAM_BR2)) && sy->nflags >= s2->nflags)
							nb_only = 0;
						break;
					}
				}
	...
	if (some_slur) {
		...
		for (sy = s1->next; sy != s2; sy = sy->next) {
			if (sy->u.note.slur_st  /* if slur start/end */ || sy->u.note.slur_end || (sy->sflags & (S_SL1 | S_SL2)))
...
static void draw_note_ties(struct SYMBOL *k1, struct SYMBOL *k2, int ntie, unsigned char *mhead1, unsigned char *mhead2, int job)
{
...
/*		case 2:  * no ending note */
		...
		if (k1->dots && !(p1 & 1) && ((s > 0 && k1->doty >= 0) || (s < 0 && k1->doty < 0)))
		...
		h = (.04 * (x2 - x1) + 10) * s;
		h *= cfmt.tieheight;
		slur_out(x1, staff_tb[staff].y + y, x2, staff_tb[staff].y + y, s, h, k1->u.note.notes[m1].ti1 & SL_DOTTED, -1);
	}
}
...
static void draw_ties(struct SYMBOL *k1, struct SYMBOL *k2, int job)
{
	struct SYMBOL *k3;
...
/* number of sharps/flats in key sig (-7 to +7) */
/* for 7 notes only */
static void setmap(int sf, unsigned char *map)
{
	int j;
...
static void draw_tblt_w(struct VOICE_S *p_voice, int nly, float y, struct tblt_s *tblt)
{
	struct SYMBOL *s;
	struct lyrics *ly;
	struct lyl *lyl;
...
static void draw_tblt_p(struct VOICE_S *p_voice, float y, struct tblt_s *tblt)
{
	...
	for (s = p_voice->sym; s; s = s->next) {
		...
		pitch = scale[pitch % 7] + acc_pitch[workmap[pitch]] + 12 * (pitch / 7) - tblt->pitch;
		octave = 0;
		while (pitch < 0) {
			pitch += 12;
			octave--;
...
/* 1: below, -1: above */
static float draw_lyrics(struct VOICE_S *p_voice, int nly, float *h, float y, int incr)
{
	int j, top;
	float sc;
...
static void draw_all_lyrics(void)
{
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
		...
		else if (p_voice->next && p_voice->next->staff == staff && p_voice->next->have_ly)
			above_tb[voice] = 1;
		if (above_tb[voice])
			lyst_tb[staff].a = 1;
		else
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
		...
		if (lyvo_tb[voice].nly > 0)
			lyst_tb[staff].bot = draw_lyrics(p_voice, lyvo_tb[voice].nly, lyvo_tb[voice].h, lyst_tb[staff].bot, 1);
		sc = staff_tb[p_voice->staff].staffscale;
		for (nly = 0; nly < 2; nly++) {
			if ((tblt = p_voice->tblts[nly]) == NULL)
				break;
			if (tblt->pitch == 0)
				draw_tblt_w(p_voice, lyvo_tb[voice].nly, lyst_tb[staff].bot * sc - tblt->hu, tblt);
			else
				draw_tblt_p(p_voice, lyst_tb[staff].bot * sc - tblt->hu, tblt);
			if (tblt->hu > 0) {
	...
	while (--i >= 0) {
		...
		staff = p_voice->staff;
		lyst_tb[staff].top = draw_lyrics(p_voice, lyvo_tb[voice].nly, lyvo_tb[voice].h, lyst_tb[staff].top, -1);
	}
...
void draw_sym_near(void)
{
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
		...
		for (s = p_voice->sym; s; s = s->next) {
			...
			if (s->abc_type != ABC_T_NOTE)
				continue;
			if (((s->sflags & S_BEAM_ST) && !(s->sflags & S_BEAM_END)) || (first_note && !(s->sflags & S_BEAM_ST))) {
				first_note = 0;
				calculate_beam(&bm, s);
			}
		}
	...
	for (s = tsfirst; s; s = s->ts_next) {
		...
		if (s->u.note.notes[0].acc) {
			y = s->y;
			if ((s->u.note.notes[0].acc & 0x07) == A_SH || s->u.note.notes[0].acc == A_NT)
				y -= 7;
			else
				y -= 5;
			if (s->ymn > y)
...
static void draw_vname(float indent)
{
	...
	for (staff = nstaff; staff >= 0; staff--) {
		...
		if (staff_p->nl == 0)
			continue;
		y = staff_tb[staff].y + staff_tb[staff].topbar * .5 * staff_tb[staff].staffscale + 9 * (staff_p->nl - 1) - cfmt.font_tb[VOICEFONT].size * .3;
		n = staff;
...
static float set_staff(void)
{
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
		if (p_voice->scale != 1)
			a2b("/scvo%d{gsave %.2f dup scale}!\n", (int) (p_voice - voice_tb), p_voice->scale);
		if (p_voice->tblts[0])
	...
	for (staff++; staff <= nstaff; staff++) {
		...
		if (staff_tb[staff].staffscale == staff_tb[prev_staff].staffscale) {
			...
			for (i = 0; i < YSTEP; i++) {
				v = staff_tb[staff].top[i] * staff_tb[staff].staffscale - staff_tb[prev_staff].bot[i] * staff_tb[prev_staff].staffscale;
				dy = (dy > v) ? dy : v;
			}
		}
...
float draw_systems(float indent)
{
	...
	for (s = tsfirst; s; s = s->ts_next) {
		...
		switch (s->type) {
		case BAR:
			if ((s->sflags & S_SECOND) || xstaff[staff] < 0)
				s->flags |= ABC_F_INVIS;
			if (s->flags & ABC_F_INVIS)
				break;
...
static void draw_symbols(struct VOICE_S *p_voice)
{
	...
	for (s = p_voice->sym; s; s = s->next) {
		...
		case NOTEREST:
			set_scale(s);
			if (s->abc_type == ABC_T_NOTE) {
				if ((s->sflags & (S_BEAM_ST | S_BEAM_END)) == S_BEAM_ST || (first_note && !(s->sflags & S_BEAM_ST))) {
					first_note = 0;
	...
	for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
		...
		for ( ; s; s = s->next) {
			...
			if (s->pits[0] < 20 && (s->u.note.notes[0].ti1 & 0x07) == SL_BELOW)
				;
			else if (s->pits[s->nhd] > 24 && (s->u.note.notes[s->nhd].ti1 & 0x07) == SL_ABOVE)
				;
			else
				continue;
			s2 = s->next;
