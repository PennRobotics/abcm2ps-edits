...
static void d_cresc(struct deco_elt *de)
{
  ...
  if (de1 && de1->prev && de1->prev->s == s && ((de->flags ^ de1->prev->flags) & DE_UP) == 0) {
    ...
    struct SYMBOL *prev)
{
  ...
  for (i = dc->n; --i >= 0; ) {
    ...
    /* special decorations */  // TODO: magic numbers
    switch (dd->func) {
      case 5:
      case 6:
      case 7:
...
//    if ((dd->flags & DE_LDST) && dd->ld_end != 0)
    ...
    /* center the dynamic marks between two staves */
    } else if ((f_staff & (1 << dd->func)) && !cfmt.dynalign && (((de->flags & DE_UP) && staff > 0) || (!(de->flags & DE_UP) && staff < nstaff))) {
      if (de->flags & DE_UP)
        ym = ymid[--staff];
      else
        ym = ymid[staff++];
      ...
      if (((de->flags & DE_UP) && y < ym) || (!(de->flags & DE_UP) && y > ym)) {
        y2 = y_get(staff, !(de->flags & DE_UP), de->x, de->val) + staff_tb[staff].y;
        if (de->flags & DE_UP)
          y2 -= dd->h;
        if (((de->flags & DE_UP) && y2 > ym) || (!(de->flags & DE_UP) && y2 < ym)) {
          y = ym;
          y_set(staff, de->flags & DE_UP, de->x, de->val, ((de->flags & DE_UP) ? y + dd->h : y) - staff_tb[staff].y);
        }
      }
    }
    ...
    if (gch->type == 'g') {      /* guitar chord */
      if (!strchr(tex_buf, '\t')) {
        ...
        for(;;) {
          n++;
          r = strchr(r, '\t');
          if (!r)  // TODO: `while(r)` in place of `for(;;)`?
            break;
          r++;
        }
        expdx = (x - s->x - w) / n;
